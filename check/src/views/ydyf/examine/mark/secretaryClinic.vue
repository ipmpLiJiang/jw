<template>
  <div class="question">
      ...
      <el-col class="q-content">
        <template>
          <el-col class="toptic-title">
            {{ detailData.username }}的自我评分为：{{
              selfScore == 0 ? "未打分" : selfScore
            ...
            <div>
              提交时间：{{
                detailDataScore[0]["content"][0]["selfSubmitTime"]
                  ? detailDataScore[0]["content"][0]["selfSubmitTime"]
                  : "暂未提交"
              }}
            </div>
          ...
          >
            <el-col class="question-title">
              <el-col :span="24" class="toptic">{{ item.title }}</el-col>
            </el-col>
            <el-col
        ...
        <template>
          <el-col class="toptic-title">
            部门负责人评分为：{{
              directorScore == 0 ? "未打分" : directorScore
            }}
            <span style="margin-left: 15px">
              部门评分等级为：{{
                directorScore > 89
            ...
            <div>
              提交时间：{{
                detailDataScore[0]["content"][0]["headSubmitTime"]
                  ? detailDataScore[0]["content"][0]["headSubmitTime"]
                  : "暂未提交"
              }}
            </div>
        ...
        <template>
          <el-col class="toptic-title">
            ...
            <div>
              提交时间：{{
                detailDataScore[0]["content"][0]["branchSubmitTime"]
                  ? detailDataScore[0]["content"][0]["branchSubmitTime"]
                  : "暂未提交"
              }}
            </div>
            ...
            >
              ...
              <el-col class="radio">
                <van-rate
                  ...
                  :count="k.maxScore"
                  :disabled="k.step == 2 || k.step == 3 ? false : true"
                  @change="onChange"
                />
              </el-col>
            </el-col>
        ...
        <el-col class="toptic-title">
          实时评优百分比优秀比上限为<span style="color: #f56c6c"
            >{{ maxPercent }}%</span
          >
        </el-col>
        <el-col>
          <div class="season-text" style="padding: 1.5rem 1rem">
            <van-progress
        ...
        <el-col class="edit-btn" v-if="!directorFlag">
          <el-button
            type="primary"
            size="small"
            @click="applyMakeSure"
    ...
    <el-dialog title="提示" :visible.sync="dialogVisible" width="30%">
      <div v-if="roleType == 7">
        您本次评分为：
        <span style="color: #409EFF; font-size: 16px"> {{ count }}分 </span>
        <br /><br />
        {{ detailData.username }}最终得分为：
        <span style="color: #409EFF; font-size: 16px"> {{ total }}分 </span>
        <br /><br />
        <div>
          <span>最终考核等级为：</span>
          <span v-if="total >= 90" style="color: #409EFF">优秀</span>
          <span v-else-if="total > 79 && total < 90" style="color: #409eff"
            >良好</span
          >
          <span v-else-if="total > 59 && total <= 79" style="color: #e6a23c"
            >一般</span
          >
          <span v-else-if="total <= 59" style="color: #f56c6c">较差</span>
        </div>
        <br />
        <div style="color: #909399">
          <div style="margin-bottom: 10px">
            D(最终得分)= A(自我评分) x 30%+ B(主任评分) x 35%+ C(书记评分) x 35%
          </div>
          <div><span style="color: #909399">优秀</span>：90 ≤ D ≤ 100</div>
          <div><span style="color: #909399">良好</span>：80 ≤ D ≤ 90</div>
          <div><span style="color: #909399">一般</span>：60 ≤ D ≤ 80</div>
          <div><span style="color: #909399">较差</span>：0 ≤ D ≤ 60</div>
        </div>
      </div>
      <span slot="footer" class="dialog-footer">
        <el-button @click="dialogVisible = false">取 消</el-button>
        <el-button type="primary" @click="submitAssess" :loading="submitLoading"
          >确 定</el-button
        >
      </span>
    </el-dialog>
    <div class="mask-show" v-if="show" @touchmove.prevent></div>
  </div>
</template>
...
export default {
  ...
  methods: {
        ...
        baseInfo(qs.stringify(data))
          .then((response) => {
            if (response.data.code == 0) {
              ...
              if (this.detailData.step == 4 || this.detailData.step == 5) {
    ...
    computeTotal() {
      for (let i = 0; i < this.detailDataScore.length; i++) {
        for (let j = 0; j < this.detailDataScore[i]["content"].length; j++) {
          //书记默认设置满分
          if (this.detailDataScore[i]["content"][j]["step"] == 2) {
    ...
    calculateScore() {
      ...
      for (let i = 0; i < this.detailDataScore.length; i++) {
        for (let j = 0; j < this.detailDataScore[i]["content"].length; j++) {
          count += Number(this.detailDataScore[i]["content"][j].branchScore);
      ...
      this.total =
        ...
        this.printFn(this.selfScore * 0.3) +
        this.printFn(this.directorScore * 0.35) +
        this.printFn(count * 0.35);
    ...
    onChange(headScore) {
      this.calculateScore();
      this.secretaryScore = this.count;
    },
    //解决精度方法
    printFn(value) {
      const precision = 14;
