<template>
  <div>
          ...
          <el-col :span="24" class="edit-btn">
            <el-form-item>
              ...
              <!-- <el-button
                ...
                @click="uploadExcel"
                ><i class="icon iconfont icon-daochu-tianchong"></i
                >上传人员excel表</el-button
              > -->
              <!-- <el-button type="info" @click="exportPerson">导出人员信息</el-button> -->
              <!-- <el-button type="info" @click="reset">重置</el-button> -->

              <!-- <a style="margin-left: 30%; color: red"
                >*请先上传人员信息,根据状态提示勾选后点击提交按钮</a
              > -->
        ...
        <el-table-column fixed="right" label="操作" width="160" align="center">
          <template slot-scope="scope">
            <el-button
              ...
              size="small"
              v-if="
                $store.state.user.user.medicalEthicsRoleList.indexOf('100') !=
                -1||$store.state.user.user.medicalEthicsRoleList.indexOf('10') !=
                -1
              "
              >编辑</el-button
            >
            ...
            <el-button
              ...
              v-if="
                $store.state.user.user.medicalEthicsRoleList.indexOf('100') !=
                -1||$store.state.user.user.medicalEthicsRoleList.indexOf('102') !=-1
              "
              >删除
            </el-button>
          </template>
    ...
    <!-- 添加人员dialog -->
    <el-dialog title="添加人员" :visible.sync="dialogFormVisible" width="30%">
      <el-form :model="modificationForm">
        ...
        <el-form-item label="发薪号" :label-width="formLabelWidth">
          <el-input
            v-model="modificationForm.moneycard"
            autocomplete="off"
        ...
        <!-- <el-form-item label="员工类型" :label-width="formLabelWidth">
          <el-select v-model="value" placeholder="请选择员工类型" @change="modification">
            <el-option v-for="item in personnelList" :key="item.value" :label="item.label" :value="item.value">
            </el-option>
          </el-select>
        </el-form-item> -->
    ...
    >
      <el-form :model="modificationForm">
        ...
        <el-form-item label="所属支部" :label-width="formLabelWidth">
          <el-cascader
            v-model="modificationForm.secretaryValue"
            :options="secretaryOptions"
            placeholder="请选择党支部"
    ...
    <!-- 上传人员excel表 -->
    <el-dialog title="提示" :visible.sync="excelDialogVisible" width="30%">
      <span class="monitor"
        >请先下载模板填好人员信息，然后上传分数excel表格</span
      >
      <span class="a-demo" @click="downloadDemo">模板下载.excel</span>
      <el-upload
      ...
      >
        <div style="text-align: left">
          <!-- <el-button type="primary" :loading="excelLoading"
            ><i class="icon iconfont icon-daochu-tianchong"></i
            >上传分数excel表</el-button
          > -->
          <el-button slot="trigger" type="primary"
            >上传人员信息excel表</el-button
          >
        </div>
      </el-upload>
      <span slot="footer" class="dialog-footer">
        <el-button @click="excelDialogVisible = false">取 消</el-button>
        <!-- <el-button type="primary" @click="confirmExcel">提 交</el-button> -->
...
import { tree, ethicsUserList } from "../../../ydyf/api/secretary/secretary";
...
export default {
  ...
  data() {
    return {
      ...
      excelActionPath:
        ...
        "medicalEthicsMsgTemp/readExcel?u_id=" +
        this.$store.state.user.user.usercode,
      dialogVisible: false,
      detailData: {},
      title: "",
  ...
  computed: {},
  created() {
    if (!this.$store.state.YdyfRole) {
      this.YdyfRole = this.$store.state.YdyfRole;
    } else {
      this.YdyfRole = JSON.parse(localStorage.getItem("YdyfRole"));
    }
  ...
  methods: {
    ...
    getList() {
      ...
      params.relationsId = this.search.relationsId;
      this.tableLoading = true;
      new Promise((response, reject) => {
        Submit(qs.stringify(params))
          .then((response) => {
    ...
    downloadDemo() {
      window.location.href =
        process.env.VUE_APP_ITEM_NAME +
        "medicalEthicsMsgTemp/downloadExcel";
    },
    //导出人员信息
    exportPerson() {
      let data = {
    ...
    editUser(row) {
      ...
      this.modificationForm.secretaryValue= [String(row.partyCommitteesId),String(row.generalBranchId),String(row.branchId)];
    ...
    //获取选中党支部
    handleChange(val) {
      // let secretaryValue = val.toString();
      if (val.length > 0) {
        this.search.relationsId = val[val.length - 1];
      }
    },
    //编辑获取选择部门
    updateHandleChange(val) {
      // let secretaryValue = val.toString();
      if (val.length > 0) {
        this.modificationForm.secretaryValue = val;
      }
    },
    ...
    redact() {
      ...
      let tSecretaryValue = this.modificationForm.secretaryValue;
      delete data.secretaryValue;
      data.relationsId = tSecretaryValue.join(',');
      new Promise((response, reject) => {
        updateMsg(qs.stringify(data))
          .then((response) => {
            if (response.data.code == 0) {
              this.$message({
                message: response.data.msg,
