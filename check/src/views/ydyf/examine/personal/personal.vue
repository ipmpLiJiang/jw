<template>
  <div>
    <h4 class="title">人员管理</h4>

    <el-row class="search">
      <el-col>
        <el-form label-width="80px" show-overflow-tooltip="true">
          <el-col :span="4">
        ...
        <el-col
          :span="10"
          style="float: right; color: red; font-size: 16px; font-weight: bold"
        >
          *请先上传人员信息,根据状态提示勾选后点击提交按钮!
        </el-col>
      </el-col>
    </el-row>
        ...
        <el-table-column fixed="right" label="操作" width="160" align="center">
          <template slot-scope="scope">
            ...
            <el-button
              ...
              type="primary"
              size="small"
              v-if="PermissionList.indexOf('100') != -1|| PermissionList.indexOf('102') != -1"
              >编辑</el-button
            >
            <el-button
    ...
    <el-dialog title="添加人员" :visible.sync="dialogFormVisible" width="30%">
      <el-form :model="modificationForm">
        ...
        <el-form-item label="所属科室" :label-width="formLabelWidth">
          <el-select
            v-model="modificationForm.departmentId"
            filterable
            placeholder="请选择"
            style="width: 78%"
          >
    ...
    <el-dialog title="提示" :visible.sync="excelDialogVisible" width="30%">
      ...
      <span slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="submitUpload" :loading="isSubmi"
          >提交</el-button
        >
      </span>
    </el-dialog>
...
import { deptListAll } from "../../api/department/department";
...
export default {
  ...
  created() {
    ...
    if (this.$store.state.YdyfRole == 100) {
  ...
  methods: {
    ...
    addUser() {
      let data = {
        ...
        departmentId: this.modificationForm.departmentId,
        personType: this.modificationForm.value,
      };
      console.log(data);
      if (!data.userName) {
      ...
      if (data.personType!==0 && data.personType!==1) {
        this.$message.warning("请选择员工类型");
        return;
      }
    ...
    //编辑
    editUser(row) {
      (this.modificationForm.secretaryValue = ""),
      ...
      this.modificationForm.secretaryValue = [
        String(row.partyCommitteesId),
        String(row.generalBranchId),
        String(row.branchId),
      ];
